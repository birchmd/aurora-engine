...
pub fn read_input_borsh<T: BorshDeserialize>() -> Result<T, ArgParseErr> {
    let bytes = read_input();
    T::try_from_slice(&bytes).map_err(|_| ArgParseErr)
}

#[cfg_attr(not(feature = "contract"), allow(dead_code))]
pub fn read_input_arr20() -> Result<[u8; 20], IncorrectInputLength> {
    unsafe {
        exports::input(INPUT_REGISTER_ID);
...
pub fn read_u64(key: &[u8]) -> Result<u64, ReadU64Error> {
