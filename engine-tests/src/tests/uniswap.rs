...
fn test_uniswap_exact_output() {
    ...
    test_utils::assert_gas_bound(profile.all_gas(), 130);
    let wasm_fraction = 100 * profile.wasm_gas() / profile.all_gas();
    assert!(wasm_fraction >= 20, "{}% not more than 20%", wasm_fraction);
    ...
    let wasm_fraction = 100 * profile.wasm_gas() / profile.all_gas();
    assert!(wasm_fraction >= 25, "{}% not more than 25%", wasm_fraction);
}
