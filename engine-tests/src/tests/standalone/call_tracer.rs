...
use aurora_engine_types::{U256, types::{Wei, Address, Yocto, NearGas}, parameters::{PromiseCreateArgs, PromiseArgs, CrossContractCallArgs}, storage};
use engine_standalone_tracing::{types::call_tracer::{self, CallTracer}, sputnik};
...
fn test_trace_contract_single_call() {
    ...
    let deploy_result = runner.submit_transaction(&signer.secret_key, deploy_tx).unwrap();
    ...
    let mut listener = CallTracer::default();
    let standalone_result = sputnik::traced_call(&mut listener, || {
        runner.submit_transaction(&signer.secret_key, tx.clone()).unwrap()
    });
    assert!(standalone_result.status.is_ok());
    assert_eq!(listener.call_stack.len(), 1);
...
fn test_trace_contract_with_precompile_sub_call() {
    ...
    let contract = test_utils::standard_precompiles::PrecompilesContract(constructor.0.deployed_at(contract_address));
...
fn test_trace_precompiles_with_subcalls() {
    ...
    runner.submit_transaction(&signer.secret_key, mint_tx).unwrap();
    let approve_tx = wnear.approve(xcc_address, U256::MAX, signer.use_nonce().into());
    runner.submit_transaction(&signer.secret_key, approve_tx).unwrap();
    // Ensure the above ERC-20 token is registered as if it were a bridged token
    {
        ...
        let mut tx = standalone::StandaloneRunner::template_tx_msg(storage, env, 0, Default::default(), &[]);
        tx.transaction = sync::types::TransactionKind::DeployErc20(
            aurora_engine::parameters::DeployErc20TokenArgs { nep141: "wrap.near".parse().unwrap( )}
        );
        ...
        let key = storage::bytes_to_key(storage::KeyPrefix::Erc20Nep141Map, wnear_address.as_bytes());
...
fn subcall_lense<'a, 'b>(root: &'a call_tracer::CallFrame, path: &'b [usize]) -> &'a call_tracer::CallFrame {
    let mut result = root;
    for index in path {
